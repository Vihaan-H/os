...
const fallbackFS={
"/home/vihaan/readme.txt":"Welcome to HindhoklaOS 2.0.\nThis is your local virtual system.",
"/home/vihaan/notes.txt":"HindhoklaOS 2.0 notes",
...
const state={fs:safeLoad("hko_fs_20",{...fallbackFS}),settings:{...defaultSettings,...safeLoad("hko_settings_20",{})},windows:new Map(),z:10,notifications:[],bootStart:performance.now(),scrolling:false,bootStarted:false,fastBoot:false};
const apps=[
...
async function boot(){
 ...
 const steps=[
 ["log","HKO kernel 2.0 initializing...",360],["hardware","Probing virtual CPU...","cpuCheck",480],["hardware","Detecting memory map...","memoryCheck",430],
 ...
 for(const s of steps){
   ...
   if(state.fastBoot&&elapsed>total*.45)break;
 }
...
function saveFS(){safeSave("hko_fs_20",state.fs)}
function notify(msg){
...
function toggleControl(k){
 ...
 safeSave("hko_settings_20",state.settings);applySettings();notify("Control center updated.")
}
...
function gitView(b){b.innerHTML='<div class="mini-card"><b>main</b><p class="muted">● Initial commit<br>│<br>● Kernel boot<br>│<br>● Desktop services<br>│<br>● 2.0 release</p></div>'}
