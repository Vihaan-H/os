...
function browser5(body){let btabs=["hko://home"],active=0;body.innerHTML="<div class=\"browser-tabs\" id=\"bTabs\"></div><div class=\"browser-bar\"><button id=\"bBack\">‹</button><button id=\"bForward\">›</button><input id=\"bUrl\" value=\"hko://home\"><button id=\"bGo\">GO</button><button id=\"bMark\">☆</button></div><div class=\"browser-page\" id=\"bPage\"></div>";const tabs=$("#bTabs",body),url=$("#bUrl",body),page=$("#bPage",body);const go=()=>{const u=url.value.trim()||"hko://home";btabs[active]=u;V5.history=[u,...V5.history.filter(x=>x!==u)].slice(0,50);v5Save();if(u==="hko://system")page.innerHTML="<h2>System</h2><div class=\"card-grid\"><div class=\"mini-card\"><b>HKO 5.0</b><small>Kernel online</small></div><div class=\"mini-card\"><b>"+esc(accountUser())+"</b><small>User</small></div></div>";else if(u==="hko://apps")page.innerHTML="<h2>Applications</h2><div class=\"card-grid\">"+apps.map(a=>"<div class=\"mini-card\"><b>"+esc(a[1])+"</b><small>"+esc(a[2])+"</small></div>").join("")+"</div>";else page.innerHTML="<div class=\"browser-home\"><div class=\"browser-logo\">H</div><h1>HINDHOKLAOS</h1><p>Local virtual web</p><button data-u=\"hko://system\">SYSTEM</button> <button data-u=\"hko://apps\">APPS</button></div>";$("[data-u]",page).forEach(b=>b.onclick=()=>{url.value=b.dataset.u;go()});tabs.innerHTML=btabs.map((x,n)=>"<button class=\"browser-tab "+(n===active?"active":"")+"\">"+esc(x)+"</button>").join("")};$("#bGo",body).onclick=go;url.onkeydown=e=>{if(e.key==="Enter")go()};$("#bMark",body).onclick=()=>{if(!V5.bookmarks.includes(url.value)){V5.bookmarks.push(url.value);v5Save();notify("Bookmark added.")}};go()}
